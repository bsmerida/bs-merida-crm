...
export function PropertyPDF({ property: p, images, agent, biz }: {
  ...
  const restImages = images.slice(1, 9); // hasta 8 más
    ...
    <Document>
      <Page size="A4" style={s.page}>
        ...
        <Text style={s.priceTag}>
          {fmtMXN(Number(p.price))}{p.operation === "Renta" ? " /mes" : ""}
        </Text>
        ...
        <View style={s.specsBox}>
          {p.bedrooms > 0 && (
            <View style={s.spec}>
              <Text style={s.specLabel}>Recámaras</Text>
              <Text style={s.specValue}>{p.bedrooms}</Text>
            </View>
          )}
          {p.bathrooms > 0 && (
            <View style={s.spec}>
              <Text style={s.specLabel}>Baños</Text>
              <Text style={s.specValue}>{p.bathrooms}</Text>
            </View>
          )}
          {p.parking > 0 && (
            <View style={s.spec}>
              <Text style={s.specLabel}>Estacionamientos</Text>
              <Text style={s.specValue}>{p.parking}</Text>
            </View>
          )}
          {p.m2_construction && (
            <View style={s.spec}>
              <Text style={s.specLabel}>m² Construcción</Text>
              <Text style={s.specValue}>{Number(p.m2_construction)}</Text>
            </View>
          )}
          {p.m2_land && (
            <View style={s.spec}>
              <Text style={s.specLabel}>m² Terreno</Text>
              <Text style={s.specValue}>{Number(p.m2_land)}</Text>
            </View>
          )}
          <View style={s.spec}>
            <Text style={s.specLabel}>Tipo</Text>
            <Text style={s.specValue}>{p.type}</Text>
          </View>
        </View>
        ...
        {p.amenities && p.amenities.length > 0 && (
          <>
            ...
            <View style={s.amenityList}>
              {p.amenities.map((a, i) => (
                <Text key={i} style={s.amenity}>• {a}</Text>
              ))}
            </View>
          </>
        )}
        ...
        {agent && (
          <>
            ...
            <Text style={s.paragraph}>
              {agent.full_name || ""}{agent.phone ? `  ·  ${agent.phone}` : ""}{agent.email ? `  ·  ${agent.email}` : ""}
            </Text>
          </>
        )}
      ...
      {/* Página de galería si hay más fotos */}
      {restImages.length > 0 && (
        <Page size="A4" style={s.page}>
          <View style={s.topBar}>
            <View>
              <Text style={s.brand}>BS | INMOBILIARIA</Text>
              <Text style={s.brandSub}>Galería  ·  {p.title}</Text>
            </View>
            {p.reference && <Text style={s.refTag}>Ref. {p.reference}</Text>}
          </View>
      ...
      {/* Página de información legal y contacto */}
      <Page size="A4" style={s.page}>
        <View style={s.topBar}>
          <Text style={s.brand}>BS | INMOBILIARIA</Text>
          {p.reference && <Text style={s.refTag}>Ref. {p.reference}</Text>}
        ...
        <Text style={s.disclaimer}>
          ...
          Todos los precios de las propiedades están sujetos a cambio y disponibilidad sin previo aviso.{"\n"}
