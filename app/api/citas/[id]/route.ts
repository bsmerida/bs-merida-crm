...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  return NextResponse.redirect(new URL(`/admin/citas/${params.id}`, process.env.NEXT_PUBLIC_SITE_URL!));
}
...
async function confirmRequest(
  ...
  // Usar agent_id de la solicitud, o fallback a Bertha
  const agentId = request.agent_id || FALLBACK_AGENT_ID;
  ...
  console.log("[GCal] Token row encontrado:", !!tokenRow, "para agentId:", agentId);
  ...
  if (tokenRow) {
    ...
    console.log("[GCal] Resultado:", gcData.id ? "OK" : "ERROR", gcData.error || "");
  } else {
    console.error("[GCal] No se encontró token para agentId:", agentId);
  }
