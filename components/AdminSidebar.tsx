    ...
    <aside className="w-60 bg-white border-r border-ink-line flex flex-col h-screen sticky top-0">
        ...
        {items.map(it => {
          ...
          return (
            <Link key={it.href} href={it.href}
              className={`w-full flex items-center gap-3 px-3 py-2 my-0.5 text-sm rounded-lg transition ${active ? "bg-brand-50 text-brand-700 font-medium" : "text-ink-muted hover:bg-ink-ghost hover:text-ink"}`}>
              <Icon name={it.icon} className="w-[18px] h-[18px]" />
              <span className="flex-1">{it.label}</span>
              {it.badge ? <span className="bg-brand-500 text-white text-[10px] rounded-full px-1.5 py-0.5 font-medium">{it.badge}</span> : null}
            </Link>
          );
        })}
      </nav>
      <div className="p-4 border-t border-ink-line">
        <div className="flex items-center gap-3 px-2 py-2 mb-2">
          <div className="w-8 h-8 rounded-full bg-brand-100 text-brand-700 flex items-center justify-center text-xs font-semibold uppercase">{initials}</div>
        ...
        <button onClick={logout} className="w-full flex items-center gap-2 px-3 py-2 text-xs text-ink-muted hover:bg-ink-ghost hover:text-ink rounded-lg transition">
