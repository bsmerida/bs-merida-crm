...
export function PropertyMapEmbed({ lat, lng, title }: Props) {
  ...
  useEffect(() => {
    ...
    const loadMap = async () => {
      ...
      // Usar Google Maps como tiles base
      const key = process.env.NEXT_PUBLIC_GOOGLE_MAPS_KEY;
      if (key) {
        L.tileLayer(
          `https://mt{s}.google.com/vt/lyrs=m&x={x}&y={y}&z={z}&key=${key}`,
  ...
  return (
    <div className="space-y-2">
      <div ref={mapRef} className="w-full rounded-2xl overflow-hidden border border-ink-line" style={{ height: "320px" }} />
