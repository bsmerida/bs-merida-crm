      ...
      {leads.length === 0 ? (
        <div className="bg-white rounded-2xl border border-ink-line p-16 text-center">
          ...
          <p className="text-sm text-ink-muted mt-2 max-w-md mx-auto">
            Cuando alguien llene el formulario de contacto del sitio web o use el chatbot, aparecerá aquí automáticamente.
          </p>
        </div>
      ) : (
        <div className="bg-white rounded-2xl border border-ink-line shadow-card overflow-hidden">
          <table className="w-full">
            <thead className="border-b border-ink-line">
              <tr>
                {["Cliente", "Contacto", "Origen", "Estado", "Asesor", "Recibido", ""].map(h => (
            ...
            <tbody>
              {leads.map(l => (
                <tr key={l.id} className="border-b border-ink-line last:border-0 hover:bg-ink-ghost/40">
                  <td className="px-5 py-4">
                    <Link href={`/admin/leads/${l.id}`} className="font-medium text-sm text-ink hover:text-brand-600">
                      {l.name}
                    </Link>
                    {l.interest && <div className="text-xs text-ink-muted mt-0.5">{l.interest}</div>}
                  </td>
                  ...
                  <td className="px-5 py-4">
                    <span className="text-xs px-2 py-1 bg-ink-ghost text-ink-muted rounded-full">{l.source || "—"}</span>
                  </td>
                  <td className="px-5 py-4">
                    <span className="text-xs px-2 py-1 rounded-full bg-brand-50 text-brand-700">{l.status}</span>
                  </td>
                  ...
                  <td className="px-5 py-4 text-right">
                    <Link href={`/admin/leads/${l.id}`} className="text-xs text-brand-600 hover:underline">
                      Ver →
                    </Link>
                  </td>
                </tr>
